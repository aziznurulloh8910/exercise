...
$data_perminggu = "SELECT WEEK(orders.orderDate) AS minggu, SUM(orderdetails.quantityOrdered * orderdetails.orderNumber) AS penjualan FROM orders INNER JOIN orderdetails ON orderdetails.orderNumber = orders.orderNumber WHERE YEAR(orders.orderDate)=".$tahun." AND MONTH(orders.orderDate)=".$bulan." GROUP BY WEEK(orders.orderDate)";

$hasilPenjualan = $conn->query($data_perminggu);
...
<div class="card shadow-sm p-2 mb-3 mt-4">
  <?php echo "<h5> Di bawah ini adalah data Tahun : " . $tahun . ", Bulan ke - ".$bulan."</h5>" ?>
</div>
<div class="card shadow mb-4">
  <div class="card-header py-3">
    <h6 class="m-0 font-weight-bold text-primary">Penjualan PerMinggu dalam Bulan</h6>
  </div>
  <div class="card-body">
...
<script>
  $('.table').DataTable();
  const tahunan = document.getElementById('chart-perbulan');
  new Chart(tahunan, {
    type: 'bar',
    data: {
      labels: [<?php echo rtrim($penjualan_label, ',') ?>],
      datasets: [{
